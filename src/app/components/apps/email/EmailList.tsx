...
const EmailList = ({ showrightSidebar }: Props) => {
  ...
  const emails = useSelector((state) =>
    getVisibleEmail(
      state.customizer.emailReducer.emails,
      state.customizer.emailReducer.currentFilter,
      state.customizer.emailReducer.emailSearch
    )
  );

  const active = useSelector((state) => state.customizer.emailReducer.emailContent);
