...
const NoteContent = ({ toggleNoteSidebar }: Props) => {
  const notelength: any = useSelector(
    (state) => state.customizer.notesReducer.notes.length-1,
  );
  const noteDetails: NotesType = useSelector(
    (state) => state.customizer.notesReducer.notes[state.customizer.notesReducer.notesContent>notelength ?  0 : state.customizer.notesReducer.notesContent],
  );
